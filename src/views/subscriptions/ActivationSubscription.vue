<template>
    <div class="subscription-listing">
        <h1>Activations des abonnements</h1>
        <div class="pt-8">
            <div class="card">
                <DataTable
                ...
                >
                    <template #header>
                        <div class="flex justify-start">
                            <IconField>
                                <Button label="Activer" icon="pi pi-thumbs-up-fill" outlined @click="confirmDeleteSelected" :disabled="!selectedSubscriptionData || !selectedSubscriptionData.length" />
                            </IconField>
                        </div>
                ...
                <Dialog v-model:visible="activeSubscribeDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
                    <div class="flex items-center gap-4">
                        <i class="pi pi-exclamation-triangle !text-3xl" />
                        <span v-if="product">Veux-tu activer ce (ces) abonnement(s)?</span>
                    </div>
                    <template #footer>
                        <Button label="Non" icon="pi pi-times" text @click="activeSubscribeDialog = false" />
                        <Button label="Oui" icon="pi pi-check" text @click="activeSelectedSubscribe" />
                    </template>
                </Dialog>
            </div>
        </div>
...
const subscriptions = computed(() => subscriptionStore.getSubscriptionsVatidate);
...
const activeSubscribeDialog = ref(false);
const product = ref({});
...
onMounted(async () => {
    if (authStore.token) {
        await subscriptionStore.findSubscriptionsValidate(authStore.token);
...
const confirmDeleteSelected = () => {
    activeSubscribeDialog.value = true;
};
const activeSelectedSubscribe = async () => {
    try {
        ...
        activeSubscribeDialog.value = false;
        ...
        await subscriptionStore.checkActivateMultiple(authStore.token, subscribesRequest.value.subscribesId);
